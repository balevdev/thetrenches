...
Network Policies require a CNI that supports them. k3s uses Flannel by default which doesn't enforce Network Policies. To enable Network Policy support in k3s, you can:

**Option 1: Install with Calico**
```bash
k3d cluster create learn-k8s-calico --port "8080:80@loadbalancer" --k3s-arg "--flannel-backend=none@server:*" --k3s-arg "--disable-network-policy@server:*"
kubectl apply -f https://docs.projectcalico.org/manifests/calico.yaml
```

**Option 2: For production**, use managed Kubernetes (EKS, GKE, AKS) which typically include CNI support.
</Callout>
